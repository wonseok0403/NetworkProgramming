...
int main(int argc, char **argv)
{
    ...
    if (argc != 5)
    {
        printf("Usage : %s [mymac][target mac][IP]\n", argv[0]);
        return 1;
    }
    ...
    // make_ip_header(&icmp_packet.ip, inet_ntoa(my_ip), argv[ARGV_TARGET_IP], 64, sizeof(struct icmp));
    ...
    for(ip=start_ip; ip<=end_ip; i++){
        dst_ip.s_addr=htonl(ip);
        make_ip_header( &icmp_packet.ip, str_my_ip, inet_ntoa(dst_ip), 64, sizeof(struct icmp));

        pcap_sendpacket(adhandle, (unsigned char*)&icmp_packet, sizeof(icmp_packet)
        );
    }
    ...
    const struct pcap_pkthdr *header, const u_char *pkt_data)
{
    ...
    if (ntohs(ether_header->ether_type) == ETHERTYPE_IP)
    {
        ...
        {
                struct icmp* icmp = (struct icmp*)((unsigned char*)ip + ip_header_len);
