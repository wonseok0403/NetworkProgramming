...
int main(int argc, char **argv)
{
    ...
    for (d = alldevs, i = 0; d; d->next)
    {
        if (no == ++i)
            break;
    }
    if (!(param_data.adhandle = pcap_open_live(d->name, 65536, 1, 1000, errbuf)))
    {
        printf("pcap_open_live_error %s\n", d->name);
    ...
    strmac_to_buffer(argv[ARGV_DEST_MAC], packet.ether_header.ether_dhost);
    strmac_to_buffer(argv[ARGV_MY_MAC], packet.ether_header.ether_shost);
    ...
    {
        make_tcp_header( &packet, inet_ntoa(my_ip), rand(),
                argv[ARGV_TARGET_IP], port,rand(), 0, TH_SYN);
                make_ip_header(&(packet.ip), inet_ntoa(my_ip), argv[ARGV_TARGET_IP], sizeof(struct tcphdr));
                pcap_sendpacket(
...
THREAD_TYPE_RETURN thread_function(THREAD_TYPE_PARAM param)
{
    ...
    pcap_loop(param_ptr->adhandle, 0, packet_handler, (unsigned char *)param);
    return 0;
}
void packet_handler(
    u_char *param,
    const struct pcap_pkthdr *header, const u_char *pk_data)
{
    unsigned char *buffer = (unsigned char *)pk_data;
