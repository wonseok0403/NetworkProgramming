...
void packet_handler(
    u_char *param, const struct pcap_pkthdr *header, const u_char *pkt_data)
{
    ...
    if ((ip->ip_p == IPPROTO_TCP) && ((ip->ip_src.s_addr == pd->filter_ip) ||
                                      (ip->ip_dst.s_addr == pd->filter_ip)))
        {
            tcphdr = (struct tcphdr *)((char *)ip + (ip->ip_hl << 2));
                    ...
                    tcphdr->th_sport == pd->filter_port) {
                        ...
                        if((tcphdr -> th_flags & TH_FIN) == TH_RST) flags[pos++] = 'R';
                        if((tcphdr -> th_flags & TH_FIN) == TH_PUSH) flags[pos++] = 'P';
                        if((tcphdr -> th_flags & TH_FIN) == TH_ACK) flags[pos++] = 'A';
                        if((tcphdr -> th_flags & TH_FIN) == TH_URG) flags[pos++] = 'U';
