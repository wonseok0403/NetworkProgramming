...
void make_tcp_header(struct tcp_packet *packet,
                     ...
                     uint32_t seq, uint32_t ack, uint8_t flag)
{
    ...
    packet->tcphdr.th_sum = cksum((unsigned short *)&(packet->ip.ip_ttl),
                                    PSEUDO_HEADER_LEN + sizeof(packet->tcphdr));
}
void make_ip_header(struct ip *ip, const char *src_ip, const char *dst_ip, uint16_t datalen)
{
    ip->ip_v = 4;
